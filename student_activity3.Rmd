...
p4 <- ggplot(D3, aes(x = x, y = y, color = gr)) +
  geom_point() +
  geom_smooth(method = "lm", fill = NA, fullrange = TRUE) +
  geom_abline(intercept = coef(mod3)[1] , slope = coef(mod3)[3], color = "black", linewidth = 1) +
  theme_bw()
print(p4)
```
...
**Pros**:  
- Coefficients are **standardized effect sizes** (“per 1-SD change in `x`”). Great for comparing predictor strength (e.g., BMI vs age).
...
**When to rescale**: For predictor comparison or publication tables. Otherwise, centering suffices for clean main effects. 