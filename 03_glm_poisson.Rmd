...
c(sum(coef(fit)), exp(sum(coef(fit))))
```
...
```{r}
fit <- glm(solved ~ studyh + class, data = dat, family = poisson(link = "log"))
summary(fit)
...
dat |> 
    ...
    ggplot(aes(x = studyh, y = solved)) +
    geom_point(aes(color = class), size = 3) +
    geom_line(aes(x = studyh, y = fit,group = class)) +
    geom_ribbon(aes(ymin = fit - 2*sqrt(fit),
                    ymax = fit + 2*sqrt(fit),
                    group = class,      
                    fill = class),
...
**REMARK:** There are multiple parametrizations of the negative-binomial distribution. The one used by `MASS` is in terms of poisson-gamma mixture see https://mc-stan.org/docs/2_20/functions-reference/nbalt.html
