...
funs <- filor::get_funs("./R/utils-glm.R")
```
...
## Why using Odds and Odds Ratios?
...
We considered a Study conducted by the University of Padova (TEDDY Child Study, 2020)[^1]. Within the study, researchers asked the participants (mothers of a young child) about the presence of post-partum depression and measured the parental stress using the PSI-Parenting Stress Index.

[^1]: Thanks to Prof. Paolo Girardi for the example, see <https://teddychild.dpss.psy.unipd.it/> for information

```{r, echo = FALSE}
teddy <- read.csv("./data/teddy_child.csv")
teddy <- teddy |> 
...
teddy$Parental_stress.=as.numeric(teddy$Parental_stress_c)*20+10
# 
tb=table(teddy$Depression_pp, teddy$Parental_stress_c) |> 
    prop.table(margin = 2)
 Dc=data.frame(Parental_stress.=sort(unique(teddy$Parental_stress.)),probs=tb[2,-1])

# Plot
teddy_plot +
    geom_bar(data=Dc,aes(x=Parental_stress.,y=probs),stat="identity",alpha=.2)  
...
```{r, echo = FALSE}
teddy_plot +geom_bar(data=Dc,aes(x=Parental_stress.,y=probs),stat="identity",alpha=.2)  +
...
plot_invlogit <- ggplot() +
    ...
    xlab(latex2exp::TeX("$X_1$")) +
    ylab(latex2exp::TeX("$logit^{-1}(p)$"))+theme_bw()
...
@Arel-Bundock2024-zl provided an overview about intepreting effects from a fitted model, these are the main elements:
...
`emmeans` is another very popular package for computing "**e**stimated **m**arginal **means**". They provide similar functionalities but `emmeans` is used more for balanced dataset usually from experimental research (e.g., balanced factorial designs) while `marginaleffects` is used more in observational research using more complex models especially with numerical variables.
...
- remember that $logit^{-1}(\beta_0 + \beta_1 X)=\frac{e^{\beta_0 + \beta_1 X}}{1 + e^{\beta_0 + \beta_1 X}}$
...
slopes(fit, newdata = data.frame(x = 2))
...
## Inverse estimation[^2]

[^2]: see also the [`investr`](https://cran.r-project.org/web/packages/investr/vignettes/introduction.pdf) package
...
```{r}
plot_param(fit, "tv_shows")+theme_bw()
```
...
**NOTE**: Notice that I use `2*se` instead of the precise quantile of the Gaussian distribution that for $\alpha = 0.05$ is $\sim 1.96$
...
fits |> 
    ...
    ylab("Probability") +
    xlab("Tv-Shows") +
    ggtitle("Predicted probabilities")+theme_bw()
```
...
::: column
```{r, echo = FALSE}
plot_param(fit, "tv_shows1")+theme_bw()
```
:::
:::::
...
$$
\log \mathcal{L}(\mu|x) = \sum^n_{i = 1} \log(f(x_i;\mu))
$$
...
lik |> 
    ...
    geom_point(x = 0, y = max(lik$lls), size = 4, col = "red")+theme_bw()
```
...
```{r, echo = FALSE}
sat_plot +theme_bw() + fit_plot+theme_bw()
```
...
```{r, echo = FALSE}
sat_plot+theme_bw() + null_plot +theme_bw()
```
...
[^4]: Adapted from <https://bookdown.org/egarpor/SSS2-UC3M/logreg-deviance.html>
...
This plot[^5] shows an example with the same residual for two different $x$ values on a Poisson GLM. Beyond the model itself, the same residual can be considered as extreme for low $x$ values and plausible for high $x$ values:

[^5]: Adapted from Dunn (2018), Fig. 8.1
...
-   for each bin we can compute the standard error as $SE = \frac{p_j (1 - p_j)}{n_j}$ where $p_j$ is the average fitted probability and $n_j$ is the number of observation in the bin $j$
...
We can see the difference for a Poisson model[^6] when using raw vs pearson residuals. The non-constant variance is controlled on the right.

[^6]: We are using a Poisson model only because the residual pattern is more clear compared to a binomial model
...
res <- ggplot(dats, aes(x = yi, y = ri)) +
    ...
    geom_point(size = 3) +
    xlab("fitted(fit)") +
    ylab("Deviance Residuals")+theme_bw()
...
qres_hist+theme_bw() + qres_fitted+theme_bw()
```
...
Where $i$ denote the parameters and standard error on a model fitted without the $i$ observation[^7]. Usually an observation is considered influential if $|DFBETAs_{i}| > \frac{2}{\sqrt{n}}$ where $n$ is the sample size.

[^7]: Note that you are not required to re-fit the model to estimate the DFBETAs, you can use the $H$ matrix, see <https://en.wikipedia.org/wiki/Influential_observation>

## Extracting influence measures

In R we can use the `influence.measures()` function to calculate each influence measure explained before[^8].

[^8]: The function actually computes also other influence measures, see Dunn (2018, section 8.8.3) for other details
...
A more relevant difference is about the residual analysis. The binary regression has different residuals compared to the binomial model fitted on the same dataset[^9].

[^9]: To note, the **binned residuals** could be considered as an attempt to mitigate the binary residuals problem by creating bins of fitted/residual values similarly to fitting a binomial model.
...
The residual deviance is also different. In fact, there is more residual deviance on the binary compared to the binomial model. However, comparing two binary and binomial models actually leads to the same conclusion. In other terms the deviance seems to be on a different scale[^10]:

[^10]: Thanks to Prof. Altoè for this suggestion
...
probit_denp1 <- ggplot() +
    ...
    geom_area(aes(x = c(-4, 4)),
              ...
              xlim = c(-4, qnorm(p1)),
              alpha = 0.5) +
    xlab(latex2exp::TeX("$\\Phi(p)$"))+theme_bw()
...
McFadden, D. (1987). Regression-based specification tests for the multinomial logit model. *Journal of Econometrics*, *34*, 63–82. [https://doi.org/10.1016/0304-4076(87)90067-4](https://doi.org/10.1016/0304-4076(87)90067-4)
